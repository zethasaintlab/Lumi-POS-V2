...
**Empat yang tersisa belum punya fiturnya, bukan auditnya:** `shift_count_attempt` (percobaan hitungan yang DITOLAK di-rollback sebelum menulis apa pun — mencatatnya menuntut jalur tulis yang bertahan melewati rollback, perubahan rancangan) · `peripheral_configured` (utang F4, menunggu shell Tauri) · `support_session_started`/`support_session_ended` (akses support, sisa Modul F).
