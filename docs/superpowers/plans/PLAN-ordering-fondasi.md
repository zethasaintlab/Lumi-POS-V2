...
- [x] **T4** — Atomisitas (FR-B2): injeksi kegagalan di tiap tahap penulisan → **nol baris tersisa di semua tabel**. Bukan hanya order yang hilang — check dan line juga.
...
- [x] **T10** — Idempotency: ketiga aturan §3.5, masing-masing satu test.
- [x] **T11** — Idempotency konkuren: dua request bersamaan dengan key sama → tepat satu `201`, satu `409`, **tepat satu order tersimpan**.
- [x] **T12** — Idempotency stress: kirim request sama 100× → tepat satu penjualan (AC `spec-b:360`).
- [x] **T13** — `outbox` ditulis dalam transaksi yang sama; rollback menghapusnya juga.
- [x] **T14** — `check` dikunci 1:1 (FR-B12): order kedua tidak bisa memakai `check` yang sama.
- [x] **T15** — Kontrak OpenAPI.
- [x] **T16** — Perbarui `CLAUDE.md`, `README.md`, `HANDOFF.md`, `modules/README.md`. **Tidak menyentuh `product/`, `research/`, `docs/superpowers/specs/`.**
