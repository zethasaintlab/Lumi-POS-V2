...
test('⛔ keranjang terbuka DIBEDAKAN dari penjualan yang dibatalkan', async () => {
  // Keduanya `status = open` di kolom. Yang satu pesanan yang sedang berjalan;
  // yang lain penjualan yang sudah dibatalkan. Menyamakannya membuat manajer
  // mengira ada pesanan menggantung yang perlu ditutup.
  ...
  assert.equal(peta.get(dibatalkan), 'dibatalkan');
  assert.equal(peta.get(terbuka), 'terbuka');
});
