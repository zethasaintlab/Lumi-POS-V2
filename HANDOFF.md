...
- [ ] **`packages/sync-client` BELUM tersambung ke apa pun.** Ia berdiri sendiri: tidak dipasang ke `apps/kasir`, tidak memakai PowerSync `writeTransaction`, dan tidak ada penjadwal yang memanggil `kirimBatch`. Port `DbLokal` dirancang agar keduanya cocok tanpa adaptor tebal, tapi **kecocokan itu belum dibuktikan dengan menjalankan kode** — hanya `node:sqlite` yang pernah mengisinya
- [ ] **Jalur naik belum diuji ujung-ke-ujung terhadap server sungguhan.** `tests/sync-client` memakai model server, bukan Fastify+PostgreSQL. Yang belum terbukti: bahwa `Idempotency-Key` yang dikirim adapter benar-benar dikenali server, dan bahwa body yang dikirim apa adanya menghasilkan hash request yang sama pada retry. `tests/dst-server` adalah tempat yang tepat untuk itu
