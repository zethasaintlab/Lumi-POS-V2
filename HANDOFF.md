...
- [ ] **Kepemilikan database lokal belum diputuskan — MEMBLOKIR seluruh sisi klien F2.** Dua keberatan yang tersisa kini **gugur keduanya**. Performa dicoret lewat pengukuran (`prototypes/03-sqlite-opfs/FINDINGS.md` §5b): VFS PowerSync justru sedikit lebih cepat, 3,25 ms vs 3,97 ms per penjualan. Kontrol transaksi dan koeksistensi skema dicoret lewat pembacaan kode sumber (§5c): `writeTransaction` adalah `BEGIN IMMEDIATE`/`COMMIT` sungguhan dengan SQL diteruskan apa adanya, dan `powersync_replace_schema` hanya menyentuh objek yang cocok `GLOB 'ps_data_*'` atau view bertanda `-- powersync-auto-generated`. **Rekomendasi: PowerSync memegang DB lokal, 20 tabel kami sebagai raw table.** Syarat mengikat: tabel kami HARUS raw table — mendeklarasikannya sebagai tabel PowerSync biasa membuat core mencoba `CREATE VIEW` bernama sama. Keputusan pemilik produk — ia menyentuh stack terkunci
- [ ] **§5c adalah pembacaan kode, BUKAN pengukuran.** Tidak satu baris pun dijalankan dengan PowerSync terpasang. Ia tidak berhak atas otoritas yang dimiliki §2, dan aturan "angka pengukuran mengalahkan estimasi" tidak berlaku untuknya. Prototipe 04 (satu `writeTransaction` berisi satu penjualan penuh di atas raw table, plus uji dua tab) adalah yang akan membuktikannya — belum dijalankan, dan butuh persetujuan menambah dependency
...
- [ ] **Pola satu-penulis (SharedWorker) belum dibangun.** Terbukti wajib, bukan opsional: tab kedua gagal membuka database sama sekali (`NoModificationAllowedError`). Tanpa pola ini, kasir yang tidak sengaja membuka tab kedua mematikan aplikasi. **Mungkin tidak perlu kami bangun**: SDK web PowerSync punya `enableMultiTabs` di atas SharedWorker (`worker/db/MultiDatabaseServer.ts`), menyala default di browser desktop kecuali Safari — belum diverifikasi dengan membuka dua tab, dan hanya berlaku bila PowerSync yang memegang DB
