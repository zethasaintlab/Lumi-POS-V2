...
- [ ] **`item_modifier_list` tidak dapat menjadi raw table — tidak punya kolom `id`, dan PowerSync mewajibkannya.** Ia katalog, jadi ia harus turun. Tiga arah ada di prototipe 04 §5 (tambah kolom `id` · turunkan sebagai bagian dokumen `item` · biarkan tabel PowerSync biasa dengan nama berbeda). Semuanya perubahan skema — **keputusan pemilik produk**, dan MEMBLOKIR replikasi turun untuk relasi item↔modifier
...
- [ ] **Jalur turun PowerSync belum pernah dijalankan.** Prototipe 04 tidak terhubung ke layanan PowerSync mana pun; yang diuji hanya database lokal dan kontrol transaksinya. Bahwa sinkronisasi sungguhan dapat menulis masuk ke raw table kami **belum dibuktikan**. Begitu juga `disconnectAndClear()` (pernyataan `clear` kami yang menentukan — salah tulis di sana menghapus data lokal) dan migrasi skema raw table
