...
- [ ] **Penjadwal relay ADA tapi tidak dinyalakan.** `buatPengirimHttp` menuntut `baseUrl`, `tenantId`, dan `actorId`; ketiganya lahir dari pendaftaran perangkat, yaitu Modul F. Menyalakannya sekarang berarti memukul endpoint tanpa identitas, 15 detik sekali, sepanjang hari. Yang perlu dilakukan saat Modul F ada: panggil `buatPenjadwal(...).mulai()` di `DbLokalProvider`, dan sambungkan pemicunya ke peristiwa `online` serta ke setiap penulisan lokal
- [ ] ⛔ **PowerSync menyimpan database lokal di IndexedDB, BUKAN OPFS.** Ditemukan 8 Agustus 2026 lewat harness pondasi kasir: `@powersync/web@2.1.1` default-nya `WASQLiteVFS.IDBBatchAtomicVFS` (`node_modules/@powersync/web/lib/db/adapters/resolveAndValidateOptions.js:32`). Prototipe 04 dan 05 tidak pernah menyetel `vfs`, jadi **seluruh angkanya adalah angka IndexedDB** — termasuk 12,33 ms per penjualan dan perbandingan 3,8× itu. Angka prototipe 03 (3,97 ms, `opfs-sahpool`) datang dari pustaka yang **berbeda** (`@sqlite.org/sqlite-wasm`) dan tidak sebanding. Opsinya: `IDBBatchAtomicVFS` (default sekarang), `OPFSCoopSyncVFS`, `AccessHandlePoolVFS`, `OPFSWriteAheadVFS`. **Keputusan pemilik produk** — ia menyentuh baris "DB lokal" di tabel stack, dan alasan COOP/COEP dipasang sejak F0. Harness melaporkan penyimpanan yang dipakai di setiap run, jadi ini tidak akan pernah lagi jadi asumsi
...
- [ ] **Interval relay 15 detik membuat tiga anak tangga backoff terbawah efektif jadi 15 detik.** spec-h:62 menulis 2/4/8/16/32/60; dengan tick tetap, yang di bawah 15 detik baru tercapai pada tick berikutnya. Arahnya selalu lebih lambat, tidak pernah lebih agresif. Pertanyaan terkait yang juga belum diputuskan: apakah kembalinya koneksi (`online`) seharusnya MERESET backoff item — pada item yang sudah 20 kali gagal, artinya menunggu 60 detik lagi setelah Wi-Fi hidup
